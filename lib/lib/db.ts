...
function initializeSchema(db: Database) {
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS toetsen (
      ...
      klas_id INTEGER NOT NULL,
      naam TEXT NOT NULL,
      type TEXT NOT NULL DEFAULT 'SO' CHECK(type IN ('SO','PW','SE','mondeling','overig')),
      datum TEXT,
      weging REAL DEFAULT 1.0,
    ...
    CREATE TABLE IF NOT EXISTS cijfers (
      ...
      opmerking TEXT,
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (toets_id) REFERENCES toetsen(id) ON DELETE CASCADE,
      FOREIGN KEY (leerling_id) REFERENCES leerlingen(id) ON DELETE CASCADE,
      UNIQUE(toets_id, leerling_id)
    );
  `);
...
function insertDemoData(db: Database) {
  ...
  // === Demo toetsen en cijfers ===

  // M3B toetsen
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (1, 'SO Werkwoordspelling', 'SO', '2026-03-20', 1.0, 'Schriftelijke overhoring hoofdstuk 3: werkwoordspelling')`);
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (1, 'PW Hoofdstuk 3+4', 'PW', '2026-04-08', 2.0, 'Proefwerk werkwoordspelling en lijdend/meewerkend voorwerp')`);
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (1, 'Boekverslag', 'SE', '2026-04-25', 3.0, 'Boekverslag van het leesboek')`);

  // H4A toetsen
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (2, 'SO Argumentatie', 'SO', '2026-03-25', 1.0, 'Overhoring argumentatiestructuur')`);
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (2, 'SE Leesvaardigheid', 'SE', '2026-04-16', 3.0, 'Schoolexamen leesvaardigheid tekst 1-4')`);

  // V5C toetsen
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (3, 'Toets Stromingen', 'PW', '2026-04-17', 2.0, 'Literatuurgeschiedenis: stromingen 20e eeuw')`);
  db.run(`INSERT INTO toetsen (klas_id, naam, type, datum, weging, omschrijving) VALUES (3, 'Mondeling Boekbespreking', 'mondeling', '2026-05-01', 3.0, 'Individueel mondeling over gelezen boek')`);

  // Demo cijfers voor M3B SO Werkwoordspelling (toets_id = 1, leerlingen 1-21)
  const cijfersM3B = [7.2, 6.5, 8.1, 5.4, 7.8, 6.9, 8.5, 7.0, 6.2, 5.8, 7.4, 6.1, 8.3, 7.6, 6.7, 9.0, 7.1, 5.5, 6.8, 8.2, 7.3];
  cijfersM3B.forEach((score, idx) => {
    db.run(`INSERT INTO cijfers (toets_id, leerling_id, score) VALUES (1, ?, ?)`, [idx + 1, score]);
  });

  // Demo cijfers voor H4A SO Argumentatie (toets_id = 4, leerlingen 22-47)
  const cijfersH4A = [7.5, 6.8, 8.2, 5.9, 7.1, 6.4, 8.7, 7.3, 6.6, 5.2, 7.9, 6.0, 8.4, 7.7, 6.3, 9.1, 7.0, 5.7, 7.4, 8.0, 6.5, 7.8, 6.9, 8.3, 7.2, 6.1];
  cijfersH4A.forEach((score, idx) => {
    db.run(`INSERT INTO cijfers (toets_id, leerling_id, score) VALUES (4, ?, ?)`, [idx + 22, score]);
  });
}
